...
function verifyInitData(initData, botToken, maxAgeSeconds = 3600) {
  ...
  for (const [key, value] of params.entries()) {
    // For bot-token HMAC validation Telegram's hash itself is excluded.
    // Other received fields, including the newer "signature" field, remain
    // part of the data-check-string.
...
module.exports = async function handler(req, res) {
  ...
  if (!initData || typeof initData !== "string") {
    return send(res, 400, { ok:false, error:"Missing initData" });
  }
